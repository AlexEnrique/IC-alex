...
\begin{flalign}
	...
	H(\{ \sigma \}) &= -J_1 \sum_{i,j} \sigma\indices{_i^j} \sigma\indices{_{i+1}^j} - J_2 \sum_{i,j} \sigma\indices{_i^j} \sigma\indices{_i^{j+1}}. \label{eq:II.3}
\end{flalign}
...
\begin{flalign*}
	...
	\ev{ \sx_i \sx_j }_q &= \frac{1}{N_y} \ev{ \qty(\sum_{k=0}^{N_y} \sigma_i^k \sigma_i^{k+1}) \qty(\sum_{k=0}^{N_y} \sigma_j^k \sigma_j^{k+1})}_{cl}, \\
...
O resultado obtido para 10000 spins se aproxima muito da solução exata do modelo de Ising \cite{Baxter, Huang}. 
